...
namespace QRCoderZpkd1_Link
{
  public partial class MainWindow : Window
  {
    ...
    public void LoadResolutions()
    {
      ...
      // Получаем переведенный плейсхолдер из менеджера языков
      string placeholderText = QRCoderZpkd1_Link.Core.LanguageManager.GetString("ResolutionPlaceholder");

      // Если ключ не найден, делаем фолбек в зависимости от текущего языка
      ...
      // Сохраняем актуальный текст плейсхолдера в поле класса
      _currentPlaceholderText = placeholderText;

      var resolutions = QRCoderZpkd1_Link.Core.ConfigManager.GetResolutions();

      // Всегда формируем свежую коллекцию для корректного отображения при смене языка
      ...
      // Привязываем коллекцию заново и принудительно устанавливаем выбор на плейсхолдер
      ResolutionComboBox.ItemsSource = _resolutionItems;
      ResolutionComboBox.SelectedIndex = 0;
      ResolutionComboBox.SelectedItem = _currentPlaceholderText;
    }
    ...
    private void SaveButton_Click(object sender, RoutedEventArgs e)
    {
      ...
      if (result == true)
      {
        try
        {
          ...
          MessageBox.Show("Карточка успешно сохранена!", "Успех", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        catch (Exception ex)
        {
